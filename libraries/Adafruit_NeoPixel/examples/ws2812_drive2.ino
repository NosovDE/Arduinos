...
#endif
#define PIN 6
#define PixNum 6 //êîëè÷åñòâî ïèêñåëåé â ëåíòå
...
{
    //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    //++++++++++++++++++ Ïåðåáîð ýôôåêòîâ â ðàçíûõ ñî÷åòàíèÿõ ñ ðàçíûìè öâåòàìè è âðåìåíåì âûïîëíåíèÿ +++++++++++++++++++++++++++++++++++++++++++++++
    //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  //++++++++++++++++ Âñå ôóíêöèè è ïàðàìåòðû ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  //FvdShadowAllPixel(0,0,0,0,0);            //Ïëàâíîå ãàøåíèå âñåõ ïèêñåëåé (çàäåðæêà, øàã ãàøåíèÿ, îòäåëüíûå öâåòà R,G,B êàæäîãî èç 3 ïèêñåëåé )
  //RevRiseAllPixel(0,0,0,0,0);              //Ïëàâíîå âêëþ÷åíèå âñåõ ïèêñåëåé (çàäåðæêà, øàã âêëþ÷åíèÿ, îòäåëüíûå öâåòà R,G,B êàæäîãî èç 3 ïèêñåëåé )
  //RevFillPixels(0,0,0,0);                  //Çàïîëíåíèå ïèêñåëåé âûáðàííûì öâåòîì â îáðàòíîì íàïðàâëåíèè (çàäåðæêà, öâåò R,G,B êàæäîãî ïèêñåëÿ)
  //FvdFillPixels(0,0,0,0);                  //Çàïîëíåíèå ïèêñåëåé âûáðàííûì öâåòîì â îáðàòíîì íàïðàâëåíèè (çàäåðæêà, öâåò R,G,B êàæäîãî ïèêñåëÿ)
  //FvdRunOnePixel(0,0,0,0);                 //Îäèí áåãóùèé ïèêñåëü â ïðÿìîì íàïðàâëåíèè (çàäåðæêà,öâåò R,G,B ïåðåìåùàåìîãî ïèêñåëÿ)
  //RevRunOnePixel(0,0,0,0);                 //Îäèí áåãóùèé ïèêñåëü â îáðàòíîì íàïðàâëåíèè (çàäåðæêà, öâåò R,G,B ïåðåìåùàåìîãî ïèêñåëÿ)
  //FvdTurnOffAllPix(0,0);                   //Ïëàâíîå ãàøåíèå ïèêñåëåé ïî î÷åðåäè â ïðÿìîì íàïðàâëåíèè (çàäåðæêà, øàã ãàøåíèÿ)
  //RevTurnOffAllPix(0,0);                   //Ïëàâíîå ãàøåíèå ïèêñåëåé ïî î÷åðåäè â îáðàòíîì íàïðàâëåíèè (çàäåðæêà, øàã ãàøåíèÿ)
  //FvdRunMixPixel(0,0,0,0);                 //Çàïîëíåíèå ëåíòû öâåòíûìè ïèêñåëÿìè âûáðàííûõ öâåòîâ â ïðÿìîì íàïðàâëåíèè (çàäåðæêà, îòäåëüíûå öâåòà R,G,B êàæäîãî èç 3 ïèêñåëåé)
  //RevRunMixPixel(0,0,0,0);                 //Çàïîëíåíèå ëåíòû öâåòíûìè ïèêñåëÿìè âûáðàííûõ öâåòîâ â ïðÿìîì íàïðàâëåíèè (çàäåðæêà, îòäåëüíûå öâåòà R,G,B êàæäîãî èç 3 ïèêñåëåé)
  //BlinkAllPix(0,0,0,0,0,0,0,0,0);          //Ìåðöàíèå âñåõ ïèêñåëåé(äëèòåëüíîñòü ìåðöàíèÿ, âðåìÿ îòêëþ÷åíèÿ, âðåìÿ ñâå÷åíèÿ, öâåò ñâå÷åíèÿ R,G,B êàæäîãî ïèêñåëÿ, öâåò ìåðöàíèÿ R,G,B êàæäîãî ïèêñåëÿ)  
  //OncomingPixels(0,0,0,0,0,0,0);           //Äâà áåãóùèõ ïèêñåëÿ âî âñòðå÷íîì íàïðàâëåíèè (çàäåðæêà,öâåò R,G,B ïåðåìåùàåìîãî ïèêñåëÿ)
  //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
}         
//***********************************************************************
...
void BlinkAllPix(uint16_t cycle, uint8_t StripOn, uint8_t StripOff, uint8_t R_on, uint8_t G_on, uint8_t B_on, uint8_t R_off, uint8_t G_off, uint8_t B_off)
  {
    uint16_t j, k;                                       //ñ÷åò÷èêè
    for(cycle; cycle>0; cycle--)                         //êîëè÷åñòâî ïîâòîðåíèé
     {
       for(j=PixNum; j>0; j--)strip.setPixelColor(j-1, R_on, G_on, B_on);    //ñâåòèì
          strip.show();                                 
          delay(StripOn); //çàäåðæêà ñâå÷åíèÿ
       for(j=PixNum; j>0; j--)strip.setPixelColor(j-1, R_off, G_off, B_off); //ãàñèì
          strip.show();
          delay(StripOff); //çàäåðæêà äî âêëþ÷åíèÿ
     }
          for(j=PixNum; j>0; j--)strip.setPixelColor(j-1, R_on, G_on, B_on); //âûõîäèì ñ âêëþ÷åííûìè ïèêñåëÿìè
          strip.show();
  }
//***********************************************************************
...
void FvdShadowAllPixel(uint8_t pause, uint8_t StepOff, uint8_t r, uint8_t g, uint8_t b)
  {
         ...
         uint16_t j, k;                          // ñ÷åò÷èêè
         float cr, cg, cb;                       // óñòàíàâëèâàåìûå öâåòà íà ëåíòå â öèêëå 
         m = max(r, g);                          // Ñàìûé ÿðêèé öâåò (áÎëüøåå çíà÷åíèå èç r, g, b)
         m = max(m, b);                    
         coeff_R = StepOff/m*r;                  // Çíà÷åíèÿ øàãîâ óìåíüøåíèÿ êàæäîãî öâåòà â ïèêñåëå
         coeff_G = StepOff/m*g;
         coeff_B = StepOff/m*b;
         for(j=PixNum; j>0; j--)
          {
          ...
          strip.show();
          cr=r; cg=g; cb=b;
         for(k=(m/StepOff/PixNum)+2; k>0; k--)   //Âíåøíèé öèêë
          {
              for(j=PixNum; j>0; j--)            //Öèêë äëÿ ïèêñåëåé â ëåíòå
               {  //Çàìåíà êàæäîãî öâåòà íà îêðóãëåííóþ ñóììó òåêóùåãî öâåòà è ÷èñëà ïðèáàâëåíèÿ
...
void RevRiseAllPixel(uint8_t pause, uint8_t StepOn, uint8_t r, uint8_t g, uint8_t b)
  {
         float coeff_R, coeff_G, coeff_B, m;    // Çíà÷åíèÿ øàãîâ óâåëè÷åíèÿ êàæäîãî öâåòà â ïèêñåëå 
         uint16_t j, k;                         // ñ÷åò÷èêè
         ...
         coeff_G = StepOn/m*g;
         coeff_B = StepOn/m*b;
         for(k=(m/StepOn/PixNum)+1; k>0; k--)   //Âíåøíèé öèêë
          {
              for(j=PixNum; j>0; j--)           //Öèêë äëÿ ïèêñåëåé â ëåíòå
               {  //Çàìåíà êàæäîãî öâåòà íà îêðóãëåííóþ ñóììó òåêóùåãî öâåòà è ÷èñëà ïðèáàâëåíèÿ
